...
\usepackage{listings, xcolor, unitsdef, hyperref}  % ! Need to fix the unities
...
\subsection{Tipo de acesso à educação}
O tipo de acesso à educação se refere a escolas públicas ou privadas. É representado por uma palavra; indivíduos que não estão em idade escolar recebem NA.
...
\subsection{Frequência cardíaca em repouso (FC)}
A frequência cardíaca em repouso é dado por um número inteiro, em bpm.
...
\subsubsection{Basófilos, Eosinófilos, Neutrófilos, Linfócitos e Monócitos}
...
Contagem de plaquetas, em milhares por microlitro (\per{mil}{\microliter}).
